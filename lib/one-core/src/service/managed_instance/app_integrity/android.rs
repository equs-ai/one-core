...
use crate::service::managed_instance::dto::AndroidBundle;
...
pub(crate) async fn validate_attestation_android(
    ...
    if keymaster_security_level != SecurityLevel::StrongBox {
        return Err(ManagedInstanceError::InsufficientSecurityLevel);
    }
...
-----END CERTIFICATE-----";
    ...
    async fn validate_attestation_insufficient_security() {
        ...
        assert!(matches!(
            result,
            Err(ManagedInstanceError::InsufficientSecurityLevel)
        ))
    }
    ...
    async fn validate_attestation_failure_mid_chain_cert_revoked() {
        ...
        assert!(matches!(
            result,
            Err(ManagedInstanceError::AppIntegrityValidationError(_))
        ));
    }
}
